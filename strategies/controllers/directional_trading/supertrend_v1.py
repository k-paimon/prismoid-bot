...
class SuperTrend(DirectionalTradingControllerBase):
    ...
    async def update_processed_data(self):
        ...
        # Backtesting fix (bare-features): pandas_ta emits complementary SUPERTl_*/SUPERTs_*
        # columns that are NaN on every row for the opposite trend direction. The backtesting
        # engine's prepare_market_data() ends with dropna(), which would drop every row and
        # produce an empty frame. Drop those helper columns and the indicator warm-up rows.
        df = df.drop(columns=[c for c in df.columns
                              if c.startswith("SUPERTl_") or c.startswith("SUPERTs_")])
        df = df.dropna(subset=[f"SUPERT_{self.config.length}_{self.config.multiplier}"])
