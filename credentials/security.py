"""
Compiled from utils/security.py (hummingbot-api).

BackendAPISecurity extends hummingbot's Security class to manage encrypted
connector credentials on disk, laid out per-account:

    <base_path>/credentials/<account_name>/
        .password_verification          # word encrypted with the config password
        conf_client.yml
        conf_fee_overrides.yml
        hummingbot_logs.yml
        connectors/<connector_name>.yml # encrypted API keys, one file per connector

Differences from the original module (kept minimal, for standalone testing):
  - imports resolve to sibling modules instead of the app's utils/ package
  - the app's `settings.app.password_verification_path` is a module constant here
"""
...
# In the app this comes from config.settings.app.password_verification_path
PASSWORD_VERIFICATION_PATH = "credentials/master_account/.password_verification"
...
class BackendAPISecurity(Security):
    ...
    def new_password_required() -> bool:
        full_path = fs_util._get_full_path(PASSWORD_VERIFICATION_PATH)
    ...
    def store_password_verification(secrets_manager: BaseSecretsManager):
        ...
        fs_util.ensure_file_and_dump_text(PASSWORD_VERIFICATION_PATH, encrypted_word)
